...
export const WORKOUT_LOCATION_TYPES = ["unknown", "indoor", "outdoor"] as const;

export type HKWorkoutActivityAlias = (typeof HK_WORKOUT_ACTIVITY_TYPES)[number];
export type WorkoutLocationType = (typeof WORKOUT_LOCATION_TYPES)[number];

export interface RecordingDataRequirements {
  duration: boolean;
  heart_rate: boolean;
  active_energy: boolean;
  distance: boolean;
  location: boolean;
}

export interface RecordingTemplate {
  canonical_activity: string;
  aliases: string[];
  hk_workout_activity_type: HKWorkoutActivityAlias;
  location_type: WorkoutLocationType;
  display_title: string;
  data_requirements: RecordingDataRequirements;
  created_at: string;
  updated_at: string;
}

interface RecordingTemplateStore {
  schema_version: typeof TEMPLATE_SCHEMA_VERSION;
  updated_at: string;
  templates: RecordingTemplate[];
}

const BUILTIN_ALIAS_GROUPS: Record<string, string[]> = {
  网球: ["网球", "tennis"],
...
function emptyStore(): RecordingTemplateStore {
  return {
...
function isDataRequirements(value: unknown): value is RecordingDataRequirements {
  if (!isObject(value)) return false;
  return (
    typeof value.duration === "boolean" &&
...
function isTemplate(value: unknown): value is RecordingTemplate {
  if (!isObject(value)) return false;
  return (
    typeof value.canonical_activity === "string" &&
    Array.isArray(value.aliases) &&
    value.aliases.every((alias) => typeof alias === "string") &&
    typeof value.hk_workout_activity_type === "string" &&
    (HK_WORKOUT_ACTIVITY_TYPES as readonly string[]).includes(value.hk_workout_activity_type) &&
    typeof value.location_type === "string" &&
    (WORKOUT_LOCATION_TYPES as readonly string[]).includes(value.location_type) &&
...
function isStore(value: unknown): value is RecordingTemplateStore {
  if (!isObject(value)) return false;
  return (
    value.schema_version === TEMPLATE_SCHEMA_VERSION &&
    typeof value.updated_at === "string" &&
...
function backupCorruptFile(path: string, reason: string): void {
  try {
    ...
    console.error(
      `[schejo] recording_templates_backup_failed reason=${reason} rename_error=${error instanceof Error ? error.message : String(error)}`
    );
  }
}

function saveStore(store: RecordingTemplateStore): void {
  const path = getTemplatePath();
  const next: RecordingTemplateStore = { ...store, updated_at: nowIso() };
  const tmp = `${path}.tmp-${process.pid}-${Date.now()}`;
  writeFileSync(tmp, JSON.stringify(next, null, 2), { encoding: "utf8", mode: 0o600 });
  renameSync(tmp, path);
}

export function loadRecordingTemplates(): RecordingTemplateStore {
  const path = getTemplatePath();
...
function builtinCanonical(value: string): string | null {
  const normalized = normalizeAlias(value);
  for (const [canonical, aliases] of Object.entries(BUILTIN_ALIAS_GROUPS)) {
    if (aliases.some((alias) => normalizeAlias(alias) === normalized)) return canonical;
  }
  return null;
}

function expandedAliases(canonicalActivity: string, aliases: string[]): string[] {
...
export function findRecordingTemplate(activityHint: string): RecordingTemplate | null {
  ...
  if (builtin) {
    ...
    for (const alias of BUILTIN_ALIAS_GROUPS[builtin] ?? []) keys.add(normalizeAlias(alias));
  }

  return (
    store.templates.find((template) => {
...
export function upsertRecordingTemplate(input: {
  ...
  const idx = store.templates.findIndex((template) =>
    [template.canonical_activity, ...template.aliases].some((alias) => aliasKeys.has(normalizeAlias(alias)))
  );
