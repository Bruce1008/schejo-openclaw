...
import {
  ...
  normalizeAlias,
  type RecordingDataRequirements,
...
export interface RecordStartCommand {
  ...
  hk_workout_activity_type: string;
...
export function parseRecordStartRequest(body: string): RecordStartRequest | null {
  if (!body.includes(RECORD_START_INTENT)) return null;
  ...
  const trigger =
    typeof rec.trigger === "string" && rec.trigger.trim() ? rec.trigger.trim() : "adhoc_declare";
  const modificationNote =
    typeof rec.modification_note === "string" && rec.modification_note.trim()
...
function generateRecordCommandId(): string {
  const shifted = new Date(Date.now() + 8 * 60 * 60 * 1000);
...
export function buildReusedRecordStartCommand(
  template: RecordingTemplate,
  opts?: { now?: string; commandId?: string; sourceIntent?: string },
): RecordStartCommand {
  return {
    schema_version: "record-start-0.1",
 ...
 */
export function shouldDeferFallback(body: string): boolean {
  return (
    body.includes(PLAN_REQUEST_INTENT) ||
 ...
 */
function findReusableTemplate(activityHint: string): RecordingTemplate | null {
 ...
 */
export function tryBuildReusedRecordStartCommand(body: string): RecordStartCommand | null {
  const request = parseRecordStartRequest(body);
  if (!request) return null;
  if (request.trigger !== "adhoc_declare") return null;
