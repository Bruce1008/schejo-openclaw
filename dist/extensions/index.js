...
function clip(text, maxLength = 500) {
    return text.length > maxLength ? text.slice(0, maxLength) : text;
}
function extractJsonCodeBlock(raw) {
...
function extractDailyReportJsonCandidates(raw) {
...
function normalizeDailyReportKeyMetrics(report, summary) {
    if (summary) {
        return {
...
function normalizeSuggestions(value, fallbacks) {
    const source = Array.isArray(value) ? value : [];
    const out = [];
    for (const item of source) {
        const text = readString(item);
...
function resolveThinSliceFallbackReply(body) {
    const isSchejoPrefixed = body.startsWith(SCHEJO_SKILL_PREFIX) ||
...
function formatReminderSection(prompt) {
...
function buildDailyReportPrompt(requestId, summary, userProfile, reminder) {
...
function prepareReminderForDailyReport() {
    let state = loadState();
...
async function postJsonForBody(url, body) {
    const response = await fetch(url, {
        method: "POST",
        headers: {
            "content-type": "application/json",
...
function assertHealthReportCloudResult(result, expectedStatus) {
    if (result.ok !== true) {
...
async function postHealthReportToCloud(body) {
    if (!runtimeState) {
        throw new Error("runtime is not paired");
    }
...
async function deliverDailyReportReplyPayload(ctx, requestId, payload) {
    ...
    if (!parsed.ok && pending?.submittedReport) {
        pendingDailyReports.delete(requestId);
        logWithContext(ctx, `[schejo] report_reply_ignored_after_tool_submit request_id=${requestId}: ${parsed.error}`);
        return {
            status: "ready",
            requestId,
            rawOutput: text,
    ...
    try {
        ...
        pendingDailyReports.delete(requestId);
        logWithContext(ctx, `[schejo] report_posted request_id=${requestId} status=${statusBody.status}`);
    }
    catch (error) {
...
async function dispatchDailyReportRequest(ctx, requestId, summary, userProfile) {
    ...
    if (reminder) {
        logWithContext(ctx, `[schejo] daily_report_reminder request_id=${requestId} kind=${reminder.reminder.kind} question_id=${reminder.questionId} injury_idx=${reminder.reminder.injury_idx ?? "n/a"}`);
    }
...
function parseUserAnswerEvent(event) {
    ...
    if (!questionId || !answer || !answeredAt)
        return null;
...
function handleUserAnswerEvent(ctx, event) {
    ...
    logWithContext(ctx, `[schejo] user_answer_applied question_id=${parsed.question_id} kind=${parsed.context.kind} injury_idx=${parsed.context.injury_idx ?? "n/a"}`);
}
class ActivePullTimeoutError extends Error {
    constructor(pluginRequestId) {
        super(`active pull timed out plugin_request_id=${pluginRequestId}`);
        this.name = "ActivePullTimeoutError";
    }
}
function waitForActivePull(pluginRequestId) {
...
function cancelPendingActivePull(pluginRequestId) {
...
function resolvePendingActivePull(summary) {
...
function asStringList(value) {
    if (!Array.isArray(value))
        return [];
    return value.map((item) => readString(item)).filter((item) => Boolean(item));
}
function renderDailyReportForChannel(report) {
...
async function sendDailyReportPrompt(promptId) {
    if (!runtimeState) {
        throw new Error("runtime is not paired");
    }
...
function createSchejoRequestPullTool() {
    return {
        ...
        description: "Request a fresh Schejo health pull from the paired iPhone and wait for the matching HealthSummary payload.",
...
function createSchejoSendDailyReportPromptTool() {
    return {
        ...
        description: "Send a visible APNs alert to the paired iPhone reminding the user to open Schejo and generate today's health report in the foreground. Use for scheduled daily reminders; do not use schejo_request_pull for that path.",
        parameters: Type.Object({
...
function createSchejoAddInjuryTool() {
    return {
        ...
        description: "Append a new injury entry to plugin-local state-0.1 (ADR 0008). Use when the user reports a fresh body issue. Sets reported_at to today and next_check_at to today+N days (default 14) when status is active.",
        parameters: Type.Object({
        ...
        async execute(_toolCallId, params) {
            ...
            const status = statusRaw === "chronic" ? "chronic" : "active";
            const daysRaw = body.next_check_at_days;
            const nextCheckAtDays = typeof daysRaw === "number" && Number.isFinite(daysRaw) && daysRaw > 0
                ? Math.round(daysRaw)
                : undefined;
            try {
...
function createSchejoChangeStatusTool() {
    return {
        ...
        description: "Change plugin-local user_state.status to one of available / sick / injured / busy / traveling / low_motivation (state-0.1, ADR 0008). Sets since to today. Optional next_check_at_days schedules a follow-up; pass null/omit to skip the follow-up.",
        parameters: Type.Object({
        ...
        async execute(_toolCallId, params) {
            ...
            if (!toRaw || !USER_STATE_STATUS_VALUES.includes(toRaw)) {
                return jsonResult({
                    status: "failed",
            ...
            const to = toRaw;
            const daysRaw = body.next_check_at_days;
            const nextCheckAtDays = daysRaw === null
                ? null
...
function createSchejoUpdateStateTool() {
    return {
        ...
        description: "Push a short-lived body signal (state-0.1 signals.body, 72h TTL, ADR 0008). Use when the user mentions a transient signal (pain / fatigue / dizziness / sickness) that doesn't yet warrant a new injury entry.",
        parameters: Type.Object({
...
function createSchejoLogWorkoutTool() {
    return {
        ...
        description: "Append one lightweight workout-plan confirmation to plugin-local state-0.2 workout_log (ADR 0008; durable, no TTL, trace for MVP-8 recent-context learning). Call exactly once per intent=workout.plan.confirm turn: action 'do' (user did the plan) or 'skip' (user skipped). Echo plan_id back from the confirm payload; title / activity_type are optional snapshots. Never call this during plan generation or for a 'modify' request.",
        parameters: Type.Object({
            plan_id: Type.String(),
        ...
        async execute(_toolCallId, params) {
            ...
            const action = rawAction === "do" ? "do" : rawAction === "skip" ? "skip" : null;
            if (!planId || !action) {
...
function createSchejoSubmitReportTool() {
    return {
        ...
        description: "Submit one generated Schejo DailyReport to cloud so the paired iPhone receives it, then return channel-ready text.",
        parameters: Type.Object({
